        ...
        for ($i = $pass_id; $i < $passengers + $pass_id; $i++) {
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)) {
                ...
                if ($row = mysqli_fetch_assoc($result)) {
                    ...
                    $stmt = mysqli_stmt_init($conn);
                    $sql = 'SELECT passenger_id FROM passenger_profile WHERE user_id=? and flight_id=? ORDER BY passenger_id DESC LIMIT 1';
                    mysqli_stmt_prepare($stmt, $sql);
                    mysqli_stmt_bind_param($stmt, 'ii', $_SESSION['userId'],$flight_id);
                    mysqli_stmt_execute($stmt);
                    $result = mysqli_stmt_get_result($stmt);
                    $row = mysqli_fetch_assoc($result);
                    $temp = $row["passenger_id"];
                    ...
                    if (!mysqli_stmt_prepare($stmt, $sql)) {
                        ...
                        mysqli_stmt_bind_param($stmt,'iisisi',$temp,$flight_id,$new_seat,$price,$class, $_SESSION['userId']);
                        mysqli_stmt_execute($stmt);
                        $flag = true;
                    }
